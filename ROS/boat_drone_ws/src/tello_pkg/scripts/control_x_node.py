...
class ControlXNode:
    ...
    def callback(self, data):
        if data.detections:  # Verifica se há alguma tag detectada
            self.distance = data.detections[0].pose.pose.pose.position.x # [metros]
        else:
            self.distance = None
