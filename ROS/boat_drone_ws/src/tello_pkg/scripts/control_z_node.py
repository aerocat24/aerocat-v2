...
class ControlZNode:
    ...
    def __init__(self):
        rospy.init_node('control_z_node', anonymous=True)
        self.subscriber = rospy.Subscriber('/tag_detections', AprilTagDetectionArray, self.callback)

        self.distance = None
        
        self.kp = 1.8339
        self.faixa_op = 0.5 # faixa de operação do controle [metros]

        self.distance_ref = 3.0 # set point [metros]
    ...
    def callback(self, data):
        if data.detections:  # Verifica se há alguma tag detectada
            self.distance = data.detections[0].pose.pose.pose.position.z # [metros]
        else:
            self.distance = None
